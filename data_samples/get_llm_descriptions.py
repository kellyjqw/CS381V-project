...
for task in tasks:
    ...
    describe in under 100 words while still being precise and informative: the start or intermediate 
    states of {task} (including but not limited to attributes like shape, size, color, consistency, etc.). 
    What does it look like when this task is incomplete or still in progress? For the person 
    engaging in this task, what might they be doing? Do not mention anything about the end state. 
    """
    ...
    prompt_done = f"""
    Suppose you are a professional video dataset annotator and have been asked to succinctly
    describe in under 100 words while still being precise and informative: the end state of 
    {task} (including but not limited to attributes like shape, size, color, consistency, etc.). 
    What does it look like when this task is done? For the person finished with this task, what 
    might they be doing?
    """
